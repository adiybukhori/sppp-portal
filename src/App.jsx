...
export default function App() {
  ...
  const fee = useMemo(() => {
    ...
    const notYet = Number(student["Subject Not Yet"] || student["Subject Not Started"] || 0);
    ...
    return {
      ...
      registrationFee: Number(student["Registration Fee"] || 0),
      convocationFee: Number(student["Convocation Fee"] || 0),
  ...
  async function loadByIc(icValue) {
    setLoading(true);
    setError("");

    try {
      const res = await fetch(API_URL + "?action=login&ic=" + encodeURIComponent(icValue));
  ...
  async function loadByStudentId(studentId) {
    setLoading(true);
    setError("");
        ...
        <Card className="w-full max-w-md rounded-3xl shadow-2xl border-white/10 bg-white/95">
          <CardContent className="p-8">
            ...
            <div className="space-y-4">
              ...
              <Button
                onClick={login}
                className="w-full rounded-2xl bg-blue-950 hover:bg-blue-900 text-white"
                disabled={loading}
              >
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <a href={student["Student Portal Link"] || "#"} target="_blank" className="rounded-full bg-slate-900 hover:bg-slate-800 text-white text-center py-3 px-6 font-semibold shadow-md">
            🌐 Student Portal
          </a>
          <a href={student["LMS Link"] || "#"} target="_blank" className="rounded-full border border-slate-200 bg-white hover:bg-slate-50 text-center py-3 px-6 font-semibold shadow-sm">
            📘 LMS (Moodle)
          </a>
          <a href={student["Schedule Link"] || "#"} target="_blank" className="rounded-full border border-slate-200 bg-white hover:bg-slate-50 text-center py-3 px-6 font-semibold shadow-sm">
            📅 Class Schedule
          </a>
          <a href={student["Complaint Form Link"] || "#"} target="_blank" className="rounded-full border border-slate-200 bg-white hover:bg-slate-50 text-center py-3 px-6 font-semibold shadow-sm">
            📝 Complaint Form
          </a>
        </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-slate-500">
                    {student["Student ID"]} · {student["IC/Passport"]}
                  </p>
                </div>
                <span className="rounded-full bg-blue-100 text-blue-700 px-4 py-1 text-sm font-semibold">
                  {student["Program"]}
                </span>
              </div>
          ...
          <Card className="rounded-3xl shadow-sm">
            <CardContent className="p-6">
              ...
              <div className="mt-5 space-y-2">
                ...
                <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${fee.paymentStatus === "Clear" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                  {fee.paymentStatus}
                </span>
              </div>
            ...
            <CardContent className="p-5 flex gap-4 items-center bg-transparent">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700">
                  Your LMS access is currently blocked. Please settle the outstanding amount or contact the Program Coordinator for assistance.
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              ...
              <div className="mb-6 rounded-3xl bg-slate-50 border border-slate-200 p-5 shadow-sm">
                ...
                <div className="mt-4 grid grid-cols-3 gap-3 text-center text-sm">
                  <MiniStat value={fee.taken} label="Completed" color="text-emerald-700" />
                  <MiniStat value={fee.ongoing} label="Ongoing" color="text-blue-700" />
                  <MiniStat value={fee.notYet} label="Not Started" color="text-slate-600" />
                </div>
              </div>
              ...
              <div className="mt-3 flex gap-2">
                <a href={student["Student Portal Link"] || "#"} target="_blank" className="rounded-full text-xs border border-slate-200 bg-white px-4 py-2 hover:bg-slate-50 shadow-sm">
                  🌐 Open Portal
                </a>
                <a href={student["Schedule Link"] || "#"} target="_blank" className="rounded-full text-xs border border-slate-200 bg-white px-4 py-2 hover:bg-slate-50 shadow-sm">
                  📅 View Schedule
                </a>
              </div>
            </CardContent>
          ...
          <div className="space-y-6">
            ...
            <Card className="rounded-3xl shadow-sm">
              <CardContent className="p-6">
                ...
                <div className="space-y-3 text-sm">
                  <Row label="Registration Fee" value={money(fee.registrationFee)} />
                  <Row label="Convocation Fee" value={money(fee.convocationFee)} />
                </div>
                <div className="mt-4 text-xs text-slate-500 border-t pt-3">
            ...
            <Card className="rounded-3xl shadow-sm">
              <CardContent className="p-6 space-y-3">
                <a href={student["Payment Link"] || "#"} target="_blank" className="block w-full rounded-full bg-blue-950 hover:bg-blue-900 text-white text-center py-3 px-6 font-semibold shadow-md">
                  Pay Outstanding Now
                </a>
                <a href={student["WhatsApp PIC"] || "#"} target="_blank" className="block w-full rounded-full border border-slate-200 bg-white hover:bg-slate-50 text-center py-3 px-6 font-semibold shadow-sm">
                  💬 Contact Admin
                </a>
